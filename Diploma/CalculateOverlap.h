...
#include "IsPointInsideATriangle.h"
#include <math.h>
#include "fstream"

double CalculatePixelOverlap(const Camera& first, const Camera& second)
{
	Coordinate3D
...
double CalculateAreaOverlap(const Camera& first, const Camera& second)
{
	Coordinate3D
		first_top_left_3d = GetPixelCoordinate(first,
			{ 0, 0 }),
...
void CalculatePixelOverlapBetweenSetOfImages(const vector<Camera>& cameras)
{
...
void CalculateAreaOverlapBetweenSetOfImages(const vector<Camera>& cameras)
{
	for (int i = 0; i < cameras.size(); i++)
	{
		for (int j = 0; j < cameras.size(); j++)
